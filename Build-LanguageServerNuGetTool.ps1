...
if (-not (Test-Path -LiteralPath $packagePath)) {
    throw "Language server NuGet tool package was not produced: $packagePath"
}

if ((Get-Item -LiteralPath $packagePath).Length -le 0) {
    throw "Language server NuGet tool package is empty: $packagePath"
}
...
Write-Host "Packaged language server NuGet tool:"
